...
type nodeinfo = {
  ...
  typ : string option;
  (* The "symbol" associated to this identifier, used by the Sourcegraph
     SCIP protocol: https://github.com/sourcegraph/scip
     Using this, we can essentially piggyback off of Sourcegraph's
     name resolution.
  *)
  scip_symbol : string option;
  (* We need this to store the ranges of certain kinds of definitions.
     The `pos` that we hold here is only the starting position of the node,
     not the span. This means that it is hard to do things such as determine what
     the range of a node truly is.
     We want to be able to do this, so we can query if a position falls within the
     range of a function, for instance. So, we store the range.
     This will only be filled in if -codegraph_scip is enabled.
  *)
...
val has_edge : node * node -> edge -> t -> bool
