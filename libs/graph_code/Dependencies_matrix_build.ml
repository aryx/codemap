...
let sort_by_count_rows_low_first xs m dm =
  xs
  |> List_.map (fun n -> (n, count_row (hashtbl_find_node dm.name_to_i n) m))
  |> Assoc.sort_by_val_lowfirst |> List_.map fst
  ...
  | Some h ->
      if Hashtbl.mem h s then
        ...
        let nodes_with_order =
          nodes
          |> List_.map (fun (s, node_kind) ->
                 match Hashtbl.find_opt horder s with
                 | None ->
                     Log.warn (fun m ->
                 ...
                 | Some n ->
                     ...
                     ((s, node_kind), n))
        in
        Assoc.sort_by_val_lowfirst nodes_with_order |> List_.map fst
      else (
    ...
    | Node (n, xs) ->
        ...
        then
          Node
            ( n,
              xs
              |> List_.map (fun (Node (n1, xs1)) ->
                     let more_brothers =
                       xs
                       |> List_.filter_map (fun (Node (n2, _)) ->
                              if n1 <> n2 then Some n2 else None)
                     in
                     aux (Node (n1, xs1)) (brothers @ more_brothers)) )
        else
          let children_nodes = xs |> List_.map (fun (Node (n, _)) -> n) in
          let config =
            Node
              ( n,
                (xs |> List_.map (fun (Node (n, _)) -> Node (n, [])))
                @ (brothers |> List_.map (fun n -> Node (n, []))) )
          in
          ...
          let score =
            ...
            |> List_.map (fun n ->
                   ...
                   (* + m.(idx).(idx) / 3 *))
            |> Assoc.sort_by_val_highfirst |> List_.map fst
          in
  ...
  let rec aux tree =
    ...
    | Node (n, xs) ->
        if List_.null xs then Node (n, [])
          ...
          let config_depth1 =
            Node (n, xs |> List_.map (function Node (n2, _) -> Node (n2, [])))
          in
          let children_nodes =
            xs |> List_.map (function Node (n2, _) -> n2)
          in
          let h_children_of_children_nodes =
            xs
            |> List_.map (function Node (n2, xs) -> (n2, xs))
