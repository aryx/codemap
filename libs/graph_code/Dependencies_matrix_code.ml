...
let basic_config g =
  Node
    (G.root, Graph_code.children G.root g |> List_.map (fun n -> Node (n, [])))

let basic_config_opti gopti =
  Node
    ( G.root,
      Graph_code_opti.children G.root gopti |> List_.map (fun n -> Node (n, []))
    )
  ...
  let rec aux tree =
    ...
    | Node (n2, xs) ->
        if n =*= n2 then
          ...
          Node (n2, succ |> List_.map (fun n -> Node (n, [])))
        else Node (n2, xs |> List_.map aux)
  in
  aux tree
...
let focus_on_node n deps_style tree dm =
  ...
  let rec aux tree =
    ...
    | Node (n2, xs) ->
        let xs = xs |> List_.filter_map aux in
...
let string_of_config_path xs =
  xs |> List_.map string_of_config_path_elem |> String.concat "/"
...
let score_upper_triangle dm exclude_nodes =
  let score = ref 0 in
  let exclude_idx =
    exclude_nodes |> List_.map (fun n -> hashtbl_find_node dm.name_to_i n)
  in
