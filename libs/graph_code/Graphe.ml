...
let nodes g = Hashtbl_.hkeys g.vertex_of_key
...
let succ k g =
  OG.succ g.og (g |> vertex_of_key k) |> List_.map (fun k -> key_of_vertex k g)

(* this seems slow on the version of ocamlgraph I currently have *)
let pred k g =
  OG.pred g.og (g |> vertex_of_key k) |> List_.map (fun k -> key_of_vertex k g)
...
let entry_nodes g =
  ...
  |> OG.Topological.iter (fun v ->
         if !finished || Hashtbl.mem hdone v then finished := true
           ...
           Stack_.push v res);
  !res |> List_.map (fun i -> key_of_vertex i g) |> List.rev
[@@profiling]

let has_edge k1 k2 g =
  let vx = g |> vertex_of_key k1 in
  let vy = g |> vertex_of_key k2 in
  OG.mem_edge g.og vx vy
...
let shortest_path k1 k2 g =
  ...
  let vertexes = vx :: (edges |> List_.map (fun edge -> OG.E.dst edge)) in
  vertexes |> List_.map (fun v -> key_of_vertex v g)
...
let strongly_connected_components g =
  ...
  let scc_array =
    scc_array_vt
    |> Array.map (fun xs -> xs |> List_.map (fun vt -> key_of_vertex vt g))
  in
