...
let all_children n g =
  let rec aux i =
    ...
    else i :: (xs |> List_.map (fun i -> aux i) |> List_.flatten)
  in
  aux (hashtbl_find g.name_to_i n) |> List_.map (fun i -> g.i_to_name.(i))
...
let adjust_graph_pack_some_children_under_dotdotdot parent to_pack g =
  ...
  let to_pack_idx =
    to_pack |> List_.map (fun n -> hashtbl_find g.name_to_i n)
  in
  let new_g =
    {
